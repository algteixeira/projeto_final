...
describe('Cannot accept an invalid Id', () => {
    it('must return you a 400 statusCode', async () => {
        ...
        request(app).put(`/api/v1/people/666a6a6a6aa6aa6aa66aaz66}`).send(peopleMock);
        const { status } = response;
        expect(status).toBe(400);
    });
})
...
describe('Throw error if Id is not found', () => {
    it('must return you a 404 statusCode', async () => {
        ...
        const response = await 
        request(app).put(`/api/v1/people/666a6a6a6aa6aa6aa66aaa66}`).send(peopleMock);
        const { status } = response;
        expect(status).toBe(400);
   });
})
...
describe('Cant accept an already used cpf', () => {
    it('must return you a 404 statusCode', async () => {
        ...
        const response = await 
        request(app).post(`/api/v1/people/`).send(peopleMockOne);
        let {status} = response;
        const {id} = response;
